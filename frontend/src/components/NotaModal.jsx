...
import { getPenjualanById, cicilPenjualan, getCicilanPenjualan, editNamaPelanggan } from '../services/api';
...
export default function NotaModal({ transaksiId, nomorHarian, onClose, onLunasSuccess, hideCicilHint }) {
  ...
  const [editNamaMode, setEditNamaMode] = useState(false);
...
${pengaturan?.footer_nota2 ? `<p class="notice" style="font-weight:900;">${pengaturan.footer_nota2}</p>` : ''}
  ...
  const simpanNamaPelanggan = async () => {
    ...
    setError('');
    try {
      await editNamaPelanggan(transaksiId, namaFinal);
            ...
            <>
              ...
              <div style={{ background: '#f8fafc', borderRadius: 10, padding: '12px 16px', marginBottom: 20, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px' }}>
                <div>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 500, color: '#1e293b' }}>{data.kasir || '—'}</div>
                </div>

                <div>
                  <div style={{ fontSize: 11, color: '#94a3b8', marginBottom: 2 }}>Pelanggan</div>
                  {editNamaMode ? (
                    <div style={{ display: 'flex', gap: 4 }}>
                      <input
                        ...
                        onKeyDown={e => { if (e.key === 'Enter' && !savingNama) simpanNamaPelanggan(); if (e.key === 'Escape') setEditNamaMode(false); }}
                        placeholder="Kosongkan = Pelanggan Umum"
                        style={{ fontSize: 13, padding: '3px 6px', border: '1px solid #bfdbfe', borderRadius: 5, width: '100%', boxSizing: 'border-box' }}
                      />
                      <button onClick={simpanNamaPelanggan} disabled={savingNama} style={{ background: '#2563eb', color: '#fff', border: 'none', borderRadius: 5, padding: '3px 8px', cursor: 'pointer', fontSize: 12 }}>✓</button>
                      <button onClick={() => setEditNamaMode(false)} style={{ background: '#f1f5f9', color: '#64748b', border: 'none', borderRadius: 5, padding: '3px 8px', cursor: 'pointer', fontSize: 12 }}>✕</button>
                    </div>
                  ) : (
                    <div style={{ fontSize: 13, fontWeight: 500, color: '#1e293b', display: 'flex', alignItems: 'center', gap: 6 }}>
                      {data.pelanggan || 'Umum'}
                      {user?.role === 'owner' && !isDibatalkan && (
                        <button onClick={() => { setNamaEdit(data.pelanggan || ''); setEditNamaMode(true); }}
                ...
                <div>
                  <div style={{ fontSize: 11, color: '#94a3b8', marginBottom: 2 }}>Metode</div>
                  <div style={{ fontSize: 13, fontWeight: 500, color: '#1e293b', textTransform: 'capitalize' }}>{data.metode_bayar}</div>
                </div>
                <div>
                  <div style={{ fontSize: 11, color: '#94a3b8', marginBottom: 2 }}>Status</div>
                  <div style={{ fontSize: 13, fontWeight: 500, color: '#1e293b', textTransform: 'capitalize' }}>
                    {data.status === 'dibatalkan' ? 'Dibatalkan' : data.status === 'lunas' ? 'Lunas' : 'Hutang'}
                  </div>
                </div>
              </div>
              ...
              {bisaBatalkan && showBatalkan && (
                <div style={{ marginTop: 16, background: '#fef2f2', border: '1.5px solid #fecaca', borderRadius: 12, padding: '16px 18px' }}>
                  ...
                  <div style={{ fontSize: 13, color: '#991b1b', marginBottom: 14 }}>
                    Aksi ini akan mengembalikan stok semua barang di transaksi #{nomorHarian ?? transaksiId} dan menandai transaksi sebagai <strong>dibatalkan</strong>. Gunakan untuk transaksi yang salah input. Aksi ini tidak dapat dibatalkan kembali.
                  </div>
